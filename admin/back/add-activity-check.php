...
if (empty($name) || empty($target) || empty($activiti) || empty($implementation) || empty($execution_time) || empty($track)) {
    header("location:../add-activity.php?empty=error");
    exit;
}
$execution_time= intval(substr($execution_time, 0, -3));


$sql = "INSERT INTO `activities` (`id`, `name`, `target`, `activity`, `implementation`, `track`, `budget`, `execution_time`, `who_it_added`, `created_at`) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
