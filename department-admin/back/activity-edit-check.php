<?php
session_start();
if (!isset($_SESSION['udepartment-admin'])) {
...
if (empty($name) || empty($target) || empty($activiti) || empty($implementation) || empty($execution_time) || empty($track)) {
    header("location:../plan-activity.php?empty=10&id=" . $id);
    exit;
}
...
if ($result->execute()) {
    header("location:../activity-edit.php?editing=20&id=" . $id);
} else {
    header("location:../activity-edit.php?error=30&id=" . $id);
}
